...
void Key_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin  = KEY5_PIN;														//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 											//设置成上拉输入
 	GPIO_Init(KEY5_PORT, &GPIO_InitStructure);													//根据设定参数初始化
	GPIO_SetBits(KEY5_PORT,KEY5_PIN);						 												//IO口输出高
	
	
	RCC_APB2PeriphClockCmd(KEY6_GPIO_CLK_ENABLE,ENABLE);								//使能端口时钟
	GPIO_InitStructure.GPIO_Pin  = KEY6_PIN;														//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 											//设置成上拉输入
 	GPIO_Init(KEY6_PORT, &GPIO_InitStructure);													//根据设定参数初始化
	GPIO_SetBits(KEY6_PORT,KEY6_PIN);						 												//IO口输出高
	
	RCC_APB2PeriphClockCmd(KEY7_GPIO_CLK_ENABLE,ENABLE);								//使能端口时钟
	GPIO_InitStructure.GPIO_Pin  = KEY7_PIN;														//引脚配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 											//设置成上拉输入
 	GPIO_Init(KEY7_PORT, &GPIO_InitStructure);													//根据设定参数初始化
	GPIO_SetBits(KEY7_PORT,KEY7_PIN);						 												//IO口输出高
}
...
{	 
	...
	if(key_up && (KEY1==0||KEY2==0||KEY3==0||KEY4==0||KEY5==0||KEY6==0||KEY7==0))
	{
		...
		if(KEY1==0)						//按键1按下，返回1
			...
			return 4;
		else if(KEY5==0)			//按键5按下，返回5
			return 5;
		else if(KEY6==0)			//按键6按下，返回6
			return 6;
		else if(KEY7==0)			//按键7按下，返回7
			return 7;
	}
	else if(KEY1==1&&KEY2==1&&KEY3==1&&KEY4==1&&KEY5==1&&KEY6==1&&KEY7==1)		//没有按键按下，松开标志位置1
	{
		key_up=1;
	}
